...
export default function AIMusicPage() {
  ...
  const [selectedSongForMerge, setSelectedSongForMerge] = useState<any | null>(null)
  ...
  const handleGenerateMusic = async () => {
    ...
    if (customMode) {
      if (instrumental) {
        if (!style || !title) {
          showToast('Custom Mode with instrumental requires Style and Title', 'warning')
          return
        }
      } else {
        if (!style || !title || !prompt) {
          showToast('Custom Mode requires Prompt, Style, and Title', 'warning')
          return
        }
      }
    } else {
      if (!prompt) {
        showToast('Please enter a music description', 'warning')
        return
      }
    }
    ...
    if (aiCredits < currentModel.credits) {
      showToast(`Insufficient AI credits! You need ${currentModel.credits} AI credits.`, 'error')
      return
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast('Failed to generate music: ' + (data.message || data.error), 'error')
  ...
  const pollMusicStatus = async (taskId: string) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.status === 'complete') {
            ...
            setIsGenerating(false)
            showAndSaveToast('Music Generated', 'Your AI music is ready to listen! 2 songs created.', 'success', '/dashboard/my-songs')
          } else if (data.status === 'failed') {
            clearInterval(pollInterval)
            setIsGenerating(false)
            showToast('Music generation failed. Please try again.', 'error')
          }
        }
      } catch (error) {
    ...
    setTimeout(() => {
      ...
      if (isGenerating) {
        setIsGenerating(false)
        showToast('Generation timed out. Please check back later.', 'warning')
      }
    }, 300000)
  }
  ...
  const handleSaveSong = async (song: any) => {
    try {
      ...
      if (data.success) {
        showToast('Song saved to My Songs!', 'success')
      } else {
        showToast('Failed to save song: ' + (data.error || 'Unknown error'), 'error')
      }
    } catch (error) {
  ...
  const handleMergeWithVideo = async (song: any) => {
    if (!videoUrl && !videoFile) {
      showToast('Please provide a video URL or upload a video file', 'warning')
      return
    }

    setIsMerging(true)
    setMergeProgress(0)
    setSelectedSongForMerge(song)
    ...
    try {
      ...
      if (videoFile) {
        ...
        videoSource = uploadData.url
        setMergeProgress(30)
      }

      // Start merging
      setMergeProgress(40)
      const response = await fetch('/api/video/merge-audio', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        showToast('Video merged with music successfully!', 'success')
        
        // Open merged video in new tab
        window.open(data.videoUrl, '_blank')
        
        // Reset
        setVideoFile(null)
        setVideoUrl('')
        setSelectedSongForMerge(null)
      } else {
        showToast('Failed to merge: ' + (data.error || 'Unknown error'), 'error')
      }
    } catch (error: any) {
      console.error('Merge error:', error)
      showToast('Failed to merge video with music: ' + error.message, 'error')
    } finally {
      setIsMerging(false)
      setMergeProgress(0)
    }
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          onClick={() => {
                            const input = document.getElementById(`video-input-${song.id}`) as HTMLInputElement
                            if (input) input.click()
                          }}
                          disabled={isMerging && selectedSongForMerge?.id === song.id}
                          className="btn-primary text-sm px-4 py-2 bg-green-600 hover:bg-green-700"
                        >
                          {isMerging && selectedSongForMerge?.id === song.id ? (
                            <>
          ...
          <div className="card p-6 bg-green-500/5 border-green-500/20">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-medium text-foreground">Add Music to Videos</p>
                  <p className="text-xs text-foreground-secondary">Click &quot;Add to Video&quot; on any generated song</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                <span className="text-green-500 font-bold">✓</span>
                <div>
                  <p className="font-medium text-foreground">FREE Merging</p>
                  <p className="text-xs text-foreground-secondary">FFmpeg processing is 100% free</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                <span className="text-green-500 font-bold">✓</span>
                <div>
                  <p className="font-medium text-foreground">R2 Storage</p>
                  <p className="text-xs text-foreground-secondary">Fast & reliable Cloudflare storage</p>
                </div>
              </div>
            </div>
          </div>
